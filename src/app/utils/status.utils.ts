...
export function statusBadgeClass(isSelected: boolean, status: ItemStatus, interactive = false): Record<string, boolean> {
  ...
  return {
    ...
    [`${shadow} border-transparent bg-status-dropped-bg-light dark:bg-status-dropped-bg-dark text-status-dropped-text-light dark:text-status-dropped-text-dark`]: isSelected && dropped && interactive,
    ...
    'bg-status-dropped-bg-light dark:bg-status-dropped-bg-dark text-status-dropped-text-light dark:text-status-dropped-text-dark': isSelected && dropped && !interactive,
  };
}
